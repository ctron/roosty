...
| 🟡 | Admin accounts API | `GET /api/v2/admin/accounts`, `GET /api/v1/admin/accounts/:id`, and `POST /api/v1/admin/accounts/:id/action` require an administrator account plus compatible admin OAuth scopes. Local/cached-remote listing, cursor links, and `silence`/`none` limit actions are available; suspension, deletion, reports, and custom roles remain missing. |
| 🟢 | Roosty admin API/UI | `/admin` and `/api/roosty/v1/admin/*` expose shared-database queue health, sanitized job diagnostics, account creation/password reset, account limits, and audit history. Browser mutations require a session-bound CSRF token; raw jobs and temporary passwords are never persisted in the audit log. |
...
| 🟡 | Follow graph | Local and remote follow/unfollow, relationships, followers, and following with cursor pagination are implemented. Cached remote account collections and counts expose accepted relationships known locally without crawling remote ActivityPub collections. Local and remote follows honor `reblogs` and `notify`; language filters remain missing. |
...
| 🟢 | `GET /api/v1/statuses/:id/context` | Visible local and cached-remote ancestors and descendants, with Mastodon-compatible anonymous and authenticated traversal limits. Durable background discovery resolves missing public/unlisted ancestors and fetches at most one replies page with five same-origin replies; unavailable or invalid context remains partial. |
...
| 🟢 | Replies | Reply targets are validated and reply metadata includes the target account mention. ActivityPub replies to cached remote Notes address and deliver to the parent author even without an explicit text mention. Unknown public/unlisted parents are resolved asynchronously, and newly cached parents transactionally repair unresolved reply links. |
...
| 🟢 | Follow graph federation | Signed inbound/outbound follows, undo, accept, and reject are persisted and delivered through retrying jobs. Automatic and manually approved/rejected inbound follows create their response jobs atomically; Follow and Undo support both common link and embedded-object forms. Mastodon and paged public ActivityPub follower/following collections include accepted local and remote relationships. Cached remote account APIs intentionally expose only relationships observed by this instance and do not crawl remote collections. |
