...
apply their named filters. Hydrated tab navigation preserves the profile identity card and tab bar
while only the selected timeline loads. Status pages render the visible bounded mixed local/remote
thread, content warnings, sensitive media, accessible descriptions, polls, quotes, and preview cards.
