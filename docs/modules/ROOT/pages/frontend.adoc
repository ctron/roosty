...
work queue at `/admin`, accounts at `/admin/accounts`, and audit log at `/admin/audit-log`. The
navigation link is projected only for accounts whose persisted `is_admin` flag is set, and the
server independently protects every admin route. Anonymous requests return through login;
authenticated non-administrators receive a forbidden response.
