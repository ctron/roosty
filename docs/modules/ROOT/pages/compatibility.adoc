...
|🟡
|Explore and trends
|Trending hashtags and statuses are available from shared, transactionally maintained caches.
Results are eventually consistent and normally refresh within `ROOSTY_TRENDS_REFRESH_INTERVAL`.
The links endpoint returns an empty collection until link-card indexing is implemented. The
public profile directory supports active/new ordering, local-only filtering, and offset
