...
|Poll notification types are missing. `admin.report` is available to administrators.
...
|🟡
|`notification` events
|Poll notification events are unavailable. `admin.report` uses the normal notification stream.
