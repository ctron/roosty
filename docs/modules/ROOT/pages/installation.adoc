...
|`roosty_federation_blocked_domains`
|Required
|YAML list of exact domains to block. Blocks include subdomains and take precedence over the allow-list.
...
|`5`
|Maximum concurrent remote media downloads per worker process; must be positive.
...
|`ROOSTY_FEDERATION_BLOCKED_DOMAINS`
|Empty
|Comma-separated exact domains to block, including their subdomains.
