...
|Required
|Registration mode: `closed` or `open`. The reserved `approval` value currently fails closed and is
advertised as disabled because approval workflows are not yet implemented.
...
|`ROOSTY_REGISTRATION_MODE`
|`closed`
|Registration mode: `closed` or `open`. The reserved `approval` value currently behaves as
`closed` until approval workflows are implemented.
